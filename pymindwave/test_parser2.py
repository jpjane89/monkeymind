...
class TestParserOperations(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sync_test_stream1 = StringIO.StringIO(
            '\x00\x12\x71\xaa' +
            '\xaa\xaa' + # [SYNC] sync packets
            '\x03' + # [PLENGTH] payload length
            '\xd4\x01\x00' + # in standby mode
        ...
        self.sync_test_stream2 = StringIO.StringIO(
            '\x00\x12\x71' +
            '\xaa\xaa' + # [SYNC] sync packets
            '\x03' + # [PLENGTH] payload length
            '\xd4\x01\x00' + # in standby mode
        ...
        self.disconnected_test_stream = StringIO.StringIO(
            '\xaa\xaa' + # [SYNC] sync packets
        ...
        self.raw_data_test_stream = StringIO.StringIO(
            '\x65\x02\x00\x64\x61' +
            '\xaa\xaa' +
            '\x04' +
            '\x80' +
        ...
        self.official_test_stream = StringIO.StringIO(
            '\xaa\xaa' + # [SYNC] sync packets
    ...
    def test_standby_mode(self):
        p = parser.VirtualParser(self.standby_test_stream)
        p.update()
        assert (p.dongle_state == 'standby')

    def test_sync(self):
        p = parser.VirtualParser(self.sync_test_stream1)
        p.update()
        assert (p.dongle_state == 'standby')
        p = parser.VirtualParser(self.sync_test_stream2)
        p.update()
        assert (p.dongle_state == 'standby')

    def test_official_test_stream(self):
        p = parser.VirtualParser(self.official_test_stream)
        p.update()
    ...
    def test_disconnected_mode(self):
        p = parser.VirtualParser(self.disconnected_test_stream)
        p.update()
        assert (p.dongle_state == 'disconnected')

    def test_rawdata_test(self):
        p = parser.VirtualParser(self.raw_data_test_stream)
        p.update()
        print p.raw_values
